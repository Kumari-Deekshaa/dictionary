...
search.addEventListener("click", async () => {
    try {
        ...
        console.log(error);
        meaningDiv.className = "text-center w-1/4 bg-gray-100 my-6 border-2 border-green-500 rounded block";
...
const displayData = (data) => {
    ...
    partOfSpeechDiv.className = "bg-gray-100 px-2 py-3 flex flex-col w-1/4 justify-center items-center border-2 border-green-500 rounded block";
    partOfSpeechHeader.innerHTML = "Part of Speech";
    partOfSpeechPara.innerHTML = partOfSpeech;
    let meaningList = ``;
    meanings.forEach((meaning, ind) => {
        meaningList += `<p class='my-3 px-4 py-1 text-md'>${ind + 1}) ${meaning.definition} </p>`;
    });
    meaningDiv.className = "text-center w-1/4 bg-gray-100 my-6 border-2 border-green-500 rounded block";
    meaningHeader.innerText = "Meanings";
    meaningDiv.innerHTML = meaningList;
